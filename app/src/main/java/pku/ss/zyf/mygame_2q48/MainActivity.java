 ...
 /**
    ...
    private void aiTurn(){
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                int result = new GameAI().aiThink(gamePlay);
                if (result == 1){
    ...
    private void gameCondition(){
        while(nowCondition != GAME_OVER && nowCondition != GAME_WAIT && winner == 0){
            if (nowCondition == GAME_START){
                ...
                initCards();
                initHold();
                bottomCardsTv.setVisibility(View.VISIBLE);
...
//                aiHold.add(myHold.get(chargePosition.get(0)));  //AI拿一张钓牌
                ...
                List<Card> tempCards = new ArrayList<>(getBottomTop(2));
